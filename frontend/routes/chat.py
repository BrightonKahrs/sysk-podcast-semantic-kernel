...
def chat():
    ...
    chat_res = requests.post(
        CHAT_URL,
        json={"session_id": session.get("session_id"), "prompt": prompt},
        headers={
...
def load_chat(session_id):
    ...
    chat_res = requests.get(
        LOAD_URL, headers={"X-User-ID": user_id}  # 🔒 secure header
    )
...
def delete_chat(session_id):
    ...
    chat_res = requests.post(
        DELETE_URL, headers={"X-User-ID": user_id}  # 🔒 secure header
    )
